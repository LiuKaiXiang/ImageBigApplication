 ...
 */
package com.yykaoo.photolibrary.photo.widget.gestureimageview;
    ...
    private void startZoom(MotionEvent e) {
        ...
        if (image.isLandscape()) {
            ...
            if (image.getDeviceOrientation() == Configuration.ORIENTATION_PORTRAIT) {
                int scaledHeight = image.getScaledHeight();
                if (scaledHeight <= canvasHeight) {
                    zoomTo = Math.max(DEFAULT_ZOOM * fitScaleVertical / currentScale, fitScaleHorizontal / currentScale);
                    float touchX = fitScaleVertical / currentScale > fitScaleHorizontal / currentScale ? e.getX() : image.getCenterX();
                    float touchY = image.getDrawable().getIntrinsicHeight() < image.getDrawable().getIntrinsicWidth() * 4 ? e.getY() : 0;
                    zoomAnimation.setTouchX(touchX);
                    zoomAnimation.setTouchY(touchY);
                } else {
