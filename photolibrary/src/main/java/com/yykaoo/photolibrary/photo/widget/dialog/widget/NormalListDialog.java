package com.yykaoo.photolibrary.photo.widget.dialog.widget;
...
import com.yykaoo.photolibrary.photo.tool.ResourceUtil;
import com.yykaoo.photolibrary.photo.tool.ViewUtil;
import com.yykaoo.photolibrary.photo.widget.dialog.entity.DialogMenuItem;
...
public class NormalListDialog extends BaseDialog {
     ...
     */
    private float cornerRadius_DP = 4;
    /**
     * title background color(标题背景颜色)
     */
    private int titleBgColor = ResourceUtil.getColor(R.color.colorPrimary);
    /**
     * title text(标题)
     */
    ...
    private void init() {
        ...
        /** LayoutAnimation */
        TranslateAnimation animation = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 2f, Animation.RELATIVE_TO_SELF, 0f,
    ...
    public void setUiBeforShow() {
        ...
        tv_title.setBackgroundDrawable(ViewUtil.cornerDrawable(titleBgColor, new float[]{radius, radius, radius, radius, 0, 0, 0, 0}));
        ...
        if (isTitleShow) {
            lv.setBackgroundDrawable(ViewUtil.cornerDrawable(lvBgColor, new float[]{0, 0, 0, 0, radius, radius, radius, radius}));
        } else {
            lv.setBackgroundDrawable(ViewUtil.cornerDrawable(lvBgColor, radius));
        }
    ...
    class ListDialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            float radius = dp2px(cornerRadius_DP);
            if (isTitleShow) {
                ll_item.setBackgroundDrawable((ViewUtil.listItemSelector(radius, Color.TRANSPARENT, itemPressColor, position == contents.size() - 1)));
            } else {
                ll_item.setBackgroundDrawable(ViewUtil.listItemSelector(radius, Color.TRANSPARENT, itemPressColor, contents.size(), position));
            }
